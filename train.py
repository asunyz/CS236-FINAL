...
"""
IMPORTANT:
Change me for weighting
"""
weights = torch.tensor([5., 5., 10., 10., 1., 1.]).to('cuda')
...
def d_logistic_loss(real_pred, fake_pred):
    if isinstance(real_pred,list):
        loss = 0
        for i in range(6):
            loss += (F.softplus(-real_pred[i]).mean() + F.softplus(fake_pred[i]).mean()) * weights[i]
        return loss
    else:
...
def g_nonsaturating_loss(fake_pred):
    if isinstance(fake_pred, list):
        loss = sum([F.softplus(-fake_pred[i]).mean() * weights[i] for i in range(6)])
    else:
...
def train(args, loader, generator, discriminator, extra, g_optim, d_optim, e_optim, g_ema, device, g_source, d_source):
    ...
    for idx in pbar:
        ...
        loss_dict["d"] = d_loss
        if isinstance(fake_pred, list):
            loss_dict["real_score"] = sum([real_pred[i].mean() * weights[i] for i in range(6)])
            loss_dict["fake_score"] = sum([fake_pred[i].mean() * weights[i] for i in range(6)])
        else:
